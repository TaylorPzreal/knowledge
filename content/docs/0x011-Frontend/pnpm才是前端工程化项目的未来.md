...
![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPFYeFJ9Q06PkqTkmo7icj8aYJtE0iben9c71fddaQ6eqanKmCYV8MVxbuA/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)

A包与B包同时依赖了C包的不同版本，**「由于同一目录下不能出现两个同名文件，所以这种情况下同一层级只能存在一个版本的包，另外一个版本还是要被嵌套依赖。」**

那么问题又来了，既然是要一个扁平化一个嵌套，那么这时候是如何确定哪一个扁平化哪一个嵌套的呢？

![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPF8ib9ZI38RzpxfmChAQkJCk8A6WZrr8ouX0iaccq5RtgIH1n4crib7YMGw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPF4jrdSDeSD7L21thuJQX5TPkAQ3Tuw9ck0WWYepwG4v3kg6fHaUCGAg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPFIcsTJibF68uSp2pRAXTUxzarcJLIbRgI7CGkXIiaZCiahY0J1PYd9476Q/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPFe8iaQic3Y1xMHd1BWUfXU9k3E24OyR5uBxVpvYYUEibxqAdL4SAqGQNaw/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPFFibyaGILkslDhR0iaE7T4HLqBntUQH5NUe5P942u5xibSJ3ltsLE1fzNQ/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1)

我们分别用`npm`与`pnpm`来安装vite对比看一下

npm
![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPFNZZ0XLib4AFrHW94eYOZqbicNwuZujiaPXO4EVdzqicp0nCgCZ6qygbjcA/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)
- 所有依赖包平铺在`node_modules`目录，包括直接依赖包以及其他次级依赖包
- 没有符号链接（软链接）

pnpm
![图片](https://mmbiz.qpic.cn/mmbiz_png/aw5KtMic7pia7ibbYXYvpdXKI5TeHlMQcPFh5K4ezpSNicd9XkjA29Y3EFibjm7tCS0slIjXNXja56KiaicVGNCbxurQg/640?wx_fmt=png&wxfrom=5&wx_lazy=1&wx_co=1)


- `node_modules`目录下只有`.pnpm`和直接依赖包，没有其他次级依赖包
- 直接依赖包的后面有符号链接（软链接）的标识
...
## Reference
1. <https://mp.weixin.qq.com/s/FCXM23JWSkPbUIGriVCPjA>
