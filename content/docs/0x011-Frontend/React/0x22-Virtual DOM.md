...
> [!Summary]
> 简单点，就是一个**普通的 JavaScript 对象(ReactElement)**，包含了 tag、props、children 三个属性。运行时通过执行 `React.createElement()` 或者 `jsx()` 获取到 VDOM，详见 [[0x21-React JSX Transform]]
> 
> VDOM优点
> 1. *体积更小，内存开销低*：原生DOM有大量的属性
> 2. *描述UI的能力更强大*：因为不支持AOT，语法不受限
> 3. *多平台支持*：可以对接不同的宿主环境（Dom, Native, Art）
> 
...
> 
> 很多人认为虚拟 DOM 最大的优势是 diff 算法，减少 JavaScript 操作真实 DOM 的带来的性能消耗。虽然这一个虚拟 DOM 带来的一个优势，但并不是全部。**虚拟 DOM 最大的优势在于抽象了原本的渲染过程，实现了跨平台的能力**，而不仅仅局限于浏览器的 DOM，可以是Android和 iOS 的原生组件，可以是小程序，也可以是各种GUI。
> 
...
## 一、Introduction
This article aims to clarify how the _virtual DOM_ is actually implemented in React.
...
At its core, the _virtual DOM_ is just a **tree of so-called `FiberNodes`**(we will refer to it as `FiberTree`). As we will see, these nodes are just **JavaScript objects** with some properties that are very cleverly leveraged.
...
The relationships between the React elements persist when it comes to `FiberNode`s. For instance, we could intuitively( [/in'tju:itivli/] 直观的) say that the `App`'s `FiberNode` will be the parent of the `<article>`'s `FiberNode`. As far as the `<h2>` and `<p>` tags are concerned, there is no parent-child relationship, but a **sibling relationship**.
...
-   `return`: arguably([/'ɑːɡjʊəblɪ/]可以说), this might be the least intuitive, but it can be thought of as the **parent** `FiberNode` - `H2FiberNode.return === ArticleFiberNode` and also `PFiberNode.return === ArticleFiberNode`(so, all the siblings have a reference to their parent `FiberNode`); an explanation as to why it is called `return` instead of `parent` can be found in [this document](https://github.com/acdlite/react-fiber-architecture#return) written by [Andrew Clark](https://github.com/acdlite); for the rest of the article, we will use `return` and `parent` interchangeably

This would be a plain visualization of the above `FiberTree`:
![](./assets/1111eadff779_6f75c85e.png)
...
![](./assets/1585c2492ca5_ef6dfeca.png)
...
![](./assets/f989072a4aaf_e5c1afdc.png)
...
![](./assets/9e5cc8d4bcc9_fba23bf7.png)
...
![](./assets/573faead88c2_ec03d452.png)
...
![[ec5a9d112eeb_0ec67268.png]]

_What is shown above is the moment after the changes have been committed to the real DOM._
...
![](./assets/fa6a657a0e2f_5756425b.png)

_Notice how the `count`'s value is now `8`._
...
2. https://medium.com/@deathmood/how-to-write-your-own-virtual-dom-ee74acc13060
3. https://medium.com/@aibolkussain/create-your-own-virtual-dom-to-understand-it-part-1-47b9b6fc6dfb
4. <https://indepth.dev/posts/1501/exploring-how-virtual-dom-is-implemented-in-react>
