...
- **容器（Container）**：一个 Webpack 构建产物，可以暴露模块给其他容器。
- **远程模块（Remote Module）**：来自另一个容器的模块，可以在当前容器中使用。
- **共享模块（Shared Module）**：在多个容器中共享的模块，可以被重写。

### 基本概念
- **Webpack构建**：一个独立项目通过 Webpack 打包编译而产生资源包
