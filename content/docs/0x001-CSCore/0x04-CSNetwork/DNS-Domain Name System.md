...
> [!Summary]
> DNS查询通常是「UDP先行」策略，先尝试使用UDP发送查询，如果响应超时或者数据量过大，客户端会尝试使用TCP重新发送查询。
> 
> **关键术语**
> 
...
> 执行域名解析的解析器是一段程序，包含在操作系统的 Socket 库中，发送消息这个操作并不是由解析器自身来执行，而是委托给操作系统内部的协议栈来执行。
...
## DNS服务
> 补充：使用HTTPS接口来实现DNS，开发者们可以解决他们争论了几十年的DNS加密问题。该系统称为基于HTTPS的DNS或DoH（尽量不要想到Homer Simpson）。

- [BIND](https://www.isc.org/downloads/bind/) - BIND is open source software that enables you to publish your Domain Name System (DNS) information on the Internet, and to resolve DNS queries for your users. [Bind9](https://bind9.net/) 
...
- [NSD](https://github.com/NLnetLabs/nsd) - NSD is an authoritative only, high performance, simple and open source name server.
...
- [SmartDNS](https://github.com/pymumu/smartdns) 国内域名通过国内的DNS服务器解析，国外域名通过国外DNS解析
- [Dnsmasq Website](https://thekelleys.org.uk/dnsmasq/doc.html) 

#### Unbound
Unbound is a validating, recursive, caching DNS resolver. It is designed to be fast and lean and incorporates modern features based on open standards. If you have any feedback, we would love to hear from you. You can learn more about Unbound by reading our [documentation](https://unbound.docs.nlnetlabs.nl/).

#### SmartDNS
SmartDNS 是一个运行在本地的 DNS 服务器，它接受来自本地客户端的 DNS 查询请求，然后从多个上游 DNS 服务器获取 DNS 查询结果，并将访问速度最快的结果返回给客户端，以此提高网络访问速度。 SmartDNS 同时支持指定特定域名 IP 地址，并高性匹配，可达到过滤广告的效果; 支持DOT，DOH，DOQ，DOH3，更好的保护隐私。
